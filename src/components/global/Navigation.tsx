...
export default function Navigation() {
  ...
  return (
    <>
      ...
      >
        <div className="flex items-center rounded-full border border-cmyk-cyan/20 bg-white/5 backdrop-blur-xl shadow-[0_20px_40px_rgba(0,0,0,0.4)] px-4 py-2 md:px-6 md:py-3">
          ...
          <nav className="hidden md:flex items-center justify-center gap-8 px-6 md:px-12">
            {navLinks.map((link) => (
              <Magnetic key={link.name} strength={0.15}>
                <Link
                  href={link.href}
                  className="font-body text-[0.65rem] font-bold uppercase tracking-[0.2em] text-warm-white/80 transition-colors duration-300 hover:text-cmyk-cyan whitespace-nowrap"
          ...
          <div className="flex-1 flex justify-end items-center gap-2 pl-4 md:pl-0">
            <Magnetic strength={0.2}>
              <Link
                href="#contact"
                className="relative inline-flex items-center justify-center px-6 py-3 overflow-hidden font-body text-[0.65rem] font-bold uppercase tracking-[0.2em] text-cmyk-key bg-cmyk-yellow rounded-full group whitespace-nowrap"
                onMouseEnter={() => setVariant("ENTER")}
                onMouseLeave={() => setVariant("default")}
              >
                <span className="relative z-10 transition-transform duration-300 group-hover:-translate-y-[150%]">START PROJECT</span>
                <span className="absolute inset-0 flex items-center justify-center translate-y-[150%] transition-transform duration-300 group-hover:translate-y-0 text-warm-white bg-cmyk-magenta z-10">
                  START PROJECT
                </span>
              </Link>
            </Magnetic>
